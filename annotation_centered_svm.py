...
def train_svm(
    x_train: np.ndarray,
    y_train: np.ndarray
) -> tuple[Pipeline, dict[str, float | list[float] | str]]:
    pos = int(y_train.sum())
    ...
    score = {
        ...
        "total_support_vectors": float(svm.n_support_.sum()),
        "class_balance_ratio": float(neg / pos) if pos > 0 else 1.0,
    }
...
def evaluate_fold(y_true: np.ndarray, y_pred) -> dict[str, float]:
...
def main():
    ...
    keys = ["acc", "bacc", "f1", "precision", "recall"]
    ...
    for test_pid in patients:
        ...
        model, score = train_svm(x_train, y_train)

        y_pred=model.predict(x_test)

        m = evaluate_fold(y_test, y_pred)
