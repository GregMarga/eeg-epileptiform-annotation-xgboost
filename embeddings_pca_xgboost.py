...
# -------------------------------------------------
# LOPO CV  (with PCA inside the fold loop)
# -------------------------------------------------
...
def main():
    ...
    all_importances = []
    explained_variances = []   # track explained variance ratio per fold
    ...
    for test_pid in patients:
        ...
        # -------------------------------------------------
        # PCA — fit ONLY on train, transform both
        # -------------------------------------------------
        scaler = StandardScaler()
        x_train_scaled = scaler.fit_transform(x_train)  # fit + transform στο train
        x_test_scaled = scaler.transform(x_test)  # μόνο transform στο test
        ...
        print(
            f"[{test_pid}] PCA {n_pca_components} components → "
            f"{explained:.1f}% of variance explained"
        )

        # -------------------------------------------------
        model, score = train_xgb(x_train_pca, y_train)

        imp_vec = np.zeros(n_pca_components)
        for k, v in score.items():
            idx = int(k[1:])   # "f6" → 6
            if idx < n_pca_components:
                imp_vec[idx] = v
    ...
    # -------------------------------------------------
    ev_arr = np.array(explained_variances)
    print(
        f"\n=== PCA explained variance (top {n_pca_components} components) ===\n"
        f"  per fold: {' | '.join(f'{v:.1f}%' for v in ev_arr)}\n"
        f"  mean={ev_arr.mean():.1f}%  std={ev_arr.std():.1f}%"
    )
    ...
    all_importances = np.array(all_importances)  # (n_folds, n_pca_components)
    mean_imp = all_importances.mean(axis=0)
    std_imp  = all_importances.std(axis=0)

    idx_sorted = np.argsort(mean_imp)[::-1]
    print(f"\n=== PCA component importance (gain) ===")
    for i in idx_sorted:
        print(f"PC{i:02d}  mean={mean_imp[i]:.6f}  std={std_imp[i]:.6f}")
    ...
    x_pos = range(len(pids))
    plt.figure(figsize=(10, 4))
    plt.bar(x_pos, accs, color=colors)
    plt.xticks(ticks=x_pos, labels=pids)
    plt.ylim(0.0, 1.0)
    plt.ylabel("Accuracy")
    plt.xlabel("Patient (sorted)")
    plt.title(f"LOPO Accuracy per Patient — PCA {n_pca_components} components (colored by PD pattern)")
