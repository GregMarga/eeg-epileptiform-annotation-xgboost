...
def train_xgb(x_train: np.ndarray, y_train: np.ndarray, sample_weight: np.ndarray | None = None):
    pos = int(y_train.sum())
...
def main():
    ...
    for test_pid in patients:
        ...
        x_test_full, y_test_full, center_sec_test, center_hmsms_test, source_edf_test = concat_patient_files(
            patient_files[test_pid]
        )

        # 5 pos + 5 neg για training
        ...
        # test mask: όλα εκτός από τα seed
        test_mask = np.ones(len(y_test_full), dtype=bool)
        test_mask[seed_idx] = False

        x_test = x_test_full[test_mask]
        y_test = y_test_full[test_mask]
        center_sec_test = center_sec_test[test_mask]
        center_hmsms_test = center_hmsms_test[test_mask]
        source_edf_test = source_edf_test[test_mask]

        # sample weights: 1.0 για τους άλλους, SEED_WEIGHT για τα seed
        n_train_other = sum(len(y) for y in y_train_list)
        n_seed = len(seed_idx)
        sample_weight = np.ones(n_train_other + n_seed, dtype=np.float32)
        ...
        model, score = train_xgb(x_train, y_train, sample_weight=sample_weight)
