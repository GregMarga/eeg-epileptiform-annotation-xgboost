...
# The first NEGATIVE seed is taken from a NON_PD segment.
# -------------------------------------------------

def make_splits(y, seg, rng, test_frac, min_test_per_class):
    """Return (test_idx, pool_idx, seed_idx) or None if too small.

    seed_idx = [positive_seed, negative_seed], where negative_seed is drawn
    from a NON_PD-segment window (falls back to any negative if none available).
    """
    ...
    if nonpd_neg:
        neg_seed = nonpd_neg[0]
    else:
        neg_seed = int(pool_neg[0])  # fallback: no NON_PD negative in the pool
