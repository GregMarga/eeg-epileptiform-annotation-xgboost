...
CSV_OUTPUT = Path("results_active_learning_annotpool_slidingtest.csv")
...
# (identical keys: X, y, ch_names, segment_type).
# -------------------------------------------------
...
# NOTE: the scaler is fit on the labeled SUPPORT (pool) and applied to the TEST
# (sliding). Pool and test come from different windowings, so a mild covariate
# shift is expected by design.
# -------------------------------------------------
...
# -------------------------------------------------
# Pool construction (annotation-centered set). No test split: the whole sliding
# set is the test. Negatives restricted to NON_PD; seed = 1 pos + 1 NON_PD neg.
# All indices refer to the POOL arrays.
# -------------------------------------------------
...
def active_learning_curve(X_pool, y_pool, X_test, y_test, rng, strategy,
    ...
    # Precompute density ONCE on the initial pool (standardized within the pool).
    dens_lookup = None
    if strategy == "active":
        p0 = np.array(pool)
...
def plot_curves(rows):
    ...
    fig.suptitle("Active learning vs random — pool: annotation-centered, "
                 "test: sliding (handcrafted features)")
    fig.tight_layout()
...
def main():
    pool_files = sorted(FEATURES_DIR.glob(f"*{POOL_SUFFIX}"))
    if not pool_files:
        raise RuntimeError(f"No *{POOL_SUFFIX} files in {FEATURES_DIR}")

    recordings = {}
    for pool_path in pool_files:
        stem = pool_path.name.replace(POOL_SUFFIX, "")
        test_path = FEATURES_DIR / f"{stem}{TEST_SUFFIX}"
