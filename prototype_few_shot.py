...
CSV_OUTPUT = Path("results_combined.csv")   # ← νέο όνομα
...
def load_features(path: Path) -> tuple[np.ndarray, np.ndarray]:
    ...
    n_samples  = X.shape[0]
    X = X.reshape(n_samples, n_channels, -1).mean(axis=1)  # (N, 16) — όλα τα features
...
def run_for_n_shot(n_shot, recordings, emb_index, feat_index) -> dict[str, float]:
    ...
    for base in recordings:
        try:
            ...
            skipped += 1
            if n_shot == 1:  # τύπωσε μόνο για n_shot=1 για να μην σπαμάρει
                print(f"  [SKIP] {base}: {e}")
...
def main():
    ...
    print("Mode: Handcrafted + LaBraM (combined)")
