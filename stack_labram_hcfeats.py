...
def load_features(path: Path) -> tuple[np.ndarray, np.ndarray]:
    ...
    n_samples = X.shape[0]
    X = X.reshape(n_samples, n_channels, -1).mean(axis=1)  # avg across channels
...
def load_combined(feat_paths: list[Path], emb_paths: list[Path]) -> tuple[np.ndarray, np.ndarray]:
    ...
    for base in common:
        ...
        X_combined = np.hstack([X_feat, X_emb])
...
def main():
    ...
    for test_pid in patients:
        ...
        x_test, y_test = load_combined(feat_index[test_pid], emb_index[test_pid])

        model = train_xgb(x_train, y_train)

        score = model.get_booster().get_score(importance_type="gain")

        labram_used = sum(1 for k in score if int(k[1:]) >= 16)  # features 16-215 είναι LaBraM
        hc_used = sum(1 for k in score if int(k[1:]) < 16)  # features 0-15 είναι handcrafted

        print(f"  HC features used: {hc_used}/16")
        print(f"  LaBraM features used: {labram_used}/200")
    ...
    plt.title("LOPO Accuracy per Patient — Handcrafted + LaBraM (colored by PD pattern)")
